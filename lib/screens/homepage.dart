...
class _homepageState extends State<homepage> {
  ...
  List<Widget> buildListButtons() {
    ...
    buttonsList.add(new ElevatedButton(
      //make the new list button first
      onPressed: () {
        widget.lists.add(doList("My List #" + widget.lists.length.toString(), widget.lists.length,
            [])); //new list with a default generated name and id and empty tasks
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) => taskpage(
                    //go to taskpage with the following arguments
      ...
      style: ButtonStyle(
          ...
          textStyle: (MaterialStateProperty.all(TextStyle(color: Color(0xff000000), fontSize: 18)))), //make it pretty
      child: Text("Add New List"),
    ));
    for (int i = widget.lists.length - 1; i > -1; i--) {
      //generate buttons for each list
      buttonsList.add(new ElevatedButton(
          //make a button
          onLongPress: () {
            //long hold will delete an item but will make a popup to confirm
          ...
          onPressed: () {
            Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) => taskpage(
                        //go to taskpage with the following arguments
                        activeID: widget.lists[i].lid,
                        lists: widget.lists))); //pass corresponding id of tapped list and the main lists object
          },
          //this passes the associated ID so we know where to read on the other side
          style: ButtonStyle(
              //just making it pretty

              backgroundColor: MaterialStateProperty.all(Color(0xffa6eca8)), //make it pretty
              textStyle:
                  (MaterialStateProperty.all(TextStyle(color: Color(0xff000000), fontSize: 18)))), //make it pretty
...
//make a dialog to warn user about deletion
  Future<void> deleteWarning(BuildContext context, int listID) async {
    ...
    showDialog(
        ...
        builder: (BuildContext ctx) {
          return AlertDialog(
            ...
            content: Text('Are you sure to delete this list? This cannot be undone'),
            actions: [
              // yes button
              TextButton(
                  onPressed: () {
                    //if confirmed run the delete list function
                    deleteList(listID);
                    setState(() {
                      //refreshscreen
                    });
